...
| <a name="input_kubernetes_pod_cidr"></a> [kubernetes\_pod\_cidr](#input\_kubernetes\_pod\_cidr) | Pod CIDR for the cluster. Azure CNI Overlay reserves one fixed /24 per node, so it must satisfy 2^(24 - prefix) >= the sum of all node pool autoscaler max-counts (e.g. /16 = 256, /14 = 1024). | `string` | `"10.101.0.0/16"` | no |
