...
| <a name="input_bastion_ssh_source_cidrs"></a> [bastion\_ssh\_source\_cidrs](#input\_bastion\_ssh\_source\_cidrs) | Source CIDRs allowed to SSH to the bastion (port 22). One ingress rule is created per CIDR. Must be non-empty when create\_bastion is true. | `list(string)` | <pre>[<br>  "0.0.0.0/0"<br>]</pre> | no |
